...
void ImageManager::AddImage(string strKey, const char* fileName, int width, int height, bool isTrans, COLORREF transColor)
{
	...
	mapImageDatas_string.insert(make_pair(strKey, image));
}
...
{
	map<string, Image*>::iterator it = mapImageDatas_string.find(strKey);
	if (it != mapImageDatas_string.end())
	{
		(it->second)->Release();
		delete (it->second);

		mapImageDatas_string.erase(it);
	}
}
